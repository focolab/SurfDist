...
#pragma omp parallel for
  for (int i=0; i<n_polys; i++) {
    ...
    const float * const curr_verts = &verts[3*i*n_rays];

    volumes[i] = polyhedron_volume(curr_dist, curr_verts, faces, n_rays, n_faces);

    // bounding boxes
    polyhedron_bbox(curr_dist, curr_point, curr_verts, n_rays, curr_bbox);
...
#pragma omp parallel for
  for (int i=0; i<n_polys; i++) {
    ...
    radius_inner[i] = bounding_radius_inner(curr_dist, curr_verts, faces, n_rays, n_faces);

    // outer and inner isotropic bounding radius
    radius_outer_isotropic[i] = bounding_radius_outer_isotropic(curr_dist, curr_verts,
                                                                n_rays,anisotropy);

    radius_inner_isotropic[i] = bounding_radius_inner_isotropic(curr_dist, curr_verts,
                                                                faces, n_rays,
                                                                n_faces, anisotropy);
  ...
  for (int i=0; i<n_polys-1; i++) {
    ...
    // compute polyverts
    polyhedron_polyverts(curr_dist, curr_point, curr_verts, n_rays, curr_polyverts);
...
#pragma omp parallel for schedule(dynamic) \
  ...
  shared(suppressed) 
    ...
    for (int neigh=0; neigh<results.size(); neigh++) {
      ...
      const float * const j_verts = &verts[3*j*n_rays];
      polyhedron_polyverts(&dist[j*n_rays], &points[3*j],
                           j_verts,n_rays, polyverts);
...
void  _COMMON_polyhedron_to_label(const float* dist, const float* points,
  ...
  for (int i = 0; i < n_polys; ++i) {
    ...
    const float * const curr_dist = &dist[i*n_rays];
    const float * const curr_verts= &verts[i*n_rays*3];
    const float * const curr_center = &points[i*3];

    // calculate the actual vertices and the bounding box
    polyhedron_bbox(curr_dist, curr_center, curr_verts, n_rays, bbox);
    polyhedron_polyverts(curr_dist, curr_center, curr_verts, n_rays, polyverts);
